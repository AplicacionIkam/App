...
import 'package:provider/provider.dart';
      import 'admin_cologne_model.dart';
      export 'admin_cologne_model.dart';
    
class AdminCologneWidget extends StatefulWidget {
  const AdminCologneWidget({super.key , required this.municipio, }) ;

  final DocumentReference? municipio;

  @override
  State<AdminCologneWidget> createState() => _AdminCologneWidgetState();
}

class _AdminCologneWidgetState extends State<AdminCologneWidget>  {
  late AdminCologneModel _model;
...
final scaffoldKey = GlobalKey<ScaffoldState>();
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => AdminCologneModel());
...
;
                ...
                final adminCologneMunicipioRecord = snapshot.data!;
...
Column(
      ...
      stream: queryColoniaRecord(queryBuilder: (coloniaRecord) => coloniaRecord.where('refMunicipio', isEqualTo: adminCologneMunicipioRecord.reference, ).orderBy('nombreCol'),),
      builder: (context, snapshot) {
        // Customize what your widget looks like when it's loading.
        if (!snapshot.hasData) {
